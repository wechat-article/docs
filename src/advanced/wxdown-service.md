...
> Credential 是微信用户的一些 cookie 相关信息，同一个微信对不用公众号有不一样的 Credential，所以每个公众号都需要先抓取 Credential 才能抓取阅读量和评论数据。
...
打开「系统设置」→「网络」→「Wi-Fi」→「代理」，将 HTTP 和 HTTPS 代理都设置为 `127.0.0.1:65000`：
